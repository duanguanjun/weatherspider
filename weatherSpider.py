...
def get_local_city():
    ...
    result = re.findall(
        '<span class="city_name"><em>(.*?)</em></span>', text, re.S)
    driver.close()
    return result[0]
...
class MyFrame(tkinter.Frame):
    def __init__(self, default_city):
        ...
        # clearbtn.bind("<Button-1>", self.clear)
        ...
        self.comboxlist.bind("<<ComboboxSelected>>", self.choose)
    ...
    def query(self, choose):
        if (len(self.citys[choose][0]) == 9):
            if (self.citys[choose][0][0] != '1' or self.citys[choose][0][1] != '0' or self.citys[choose][0][2] != '1'):
                ...
                weather = re.findall(
                    '<li class="date-.*?".*?".*?">(.*?)</.*?"date-i.*?">(.*?)<.*?', html2, re.S)
                ...
                if len(temp_weather) < 7:
                    # 当天
                    today1 = re.findall(
                        '<li class="blue-item active".*?>(.*?)<div class="item-active"></div>\\n</li>',
                        html2, re.S)
                ...
                b = '查询城市为：' + \
                    str(self.citys[choose][3]) + '    ' + \
                    str(self.citys[choose][1])
                self.result.insert(tkinter.END, b)
                if len(temp_weather) < 7:  # 如日本
                    if len(weather) <= 0 or len(Lowtempture) <= 0 or len(Hightempture) <= 0 or \
                            len(Lowtempture[0]) != 7 or len(Hightempture[0]) != 7:
                        a = '系统出错，数据不完整:'
                        self.result.insert(tkinter.END, a)
                    ...
                    if len(temp_weather) <= 0 or len(Lowtempture) <= 0 or len(Hightempture) <= 0 or \
                            len(Lowtempture[0]) != 7 or len(Hightempture[0]) != 7:
                        a = '系统出错，数据不完整:'
                        self.result.insert(tkinter.END, a)
                        self.result.insert(tkinter.END, url2)
                ...
                self.result.insert(tkinter.END, b)
                if len(weather) <= 0 or len(Lowtempture) <= 0 or len(Hightempture) <= 0 or \
                        len(Lowtempture[0]) != 8 or len(Hightempture[0]) != 8:
                    a = '系统出错，数据不完整:'
                    self.result.insert(tkinter.END, a)
                    self.result.insert(tkinter.END, url2)
                    print(url2)
        ...
        if (len(self.citys[choose][0]) == 12):  # 查询搜索相关结果的下一个城市天气预报
            url2 = 'http://forecast.weather.com.cn/town/weathern/' + \
                self.citys[choose][0] + '.shtml'
            responseweather = requests.get(url2, headers=self.headers)
            html2 = responseweather.content.decode('utf-8')

            weather = re.findall(
                '<li class="date-.*?".*?"da.*?">(.*?)</.*?"date-i.*?">(.*?)<.*?', html2, re.S)
